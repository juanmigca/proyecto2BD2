...
def generate_restaurants(num_restaurants, menuItems):
    restaurants = []
    cuisines = ['Italiana', 'Mexicana', 'China', 'Japonesa', 'Española', 'India','Americana', 'Guatemalteca']
...
def generate_users_orders_reviews(num_users, restaurants):
    ...
    for i in range(num_users):
        ...
        for j in range(user['numOrders']):
            restaurant_ordered = random.choice(restaurants)
            order = {
                'id': len(orders) + 1,
                'userId': user['id'],
                ...
                'restaurantId': restaurant_ordered['id'],
                'items': random.sample(restaurant_ordered['menuItems'], random.randint(1, 5))
            }
